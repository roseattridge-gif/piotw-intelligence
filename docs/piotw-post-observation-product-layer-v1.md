# PIOTW post-observation product layer v1
...
Event prediction is a separate, preregistered engine trained only after the evidence substrate and feature definitions are ready. Predictions require a named event, defined horizon, calibrated probability, frozen inputs and immutable provenance. They must remain visually distinct from evidence and interpretation.

## Product boundary

No arbitrary weights, composite scores, Pressure, Expansion, ranking or predictive meaning are authorised here. The next implementation decision follows the independent 0.3.7 observation-layer result.
