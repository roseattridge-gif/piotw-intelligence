...
test("portfolio copy rejects fake scoring and separates real from simulated data", async () => {
  const source = await readFile(new URL("../app/portfolio/page.tsx", import.meta.url), "utf8");
  assert.match(source, /real companies/);
  const demo = await readFile(new URL("../app/portfolio/demo/page.tsx", import.meta.url), "utf8");
  assert.match(demo, /synthetic|Synthetic/);
