...
from piotw_compare import ComparisonEngine
...
class UnknownCompanyOrchestrator:
    ...
    def _assemble(
        ...
        comparison_results = []
        raw_records = [{"source_record_id": record.source_record_id,
                        "family_id": record.source_family,
                        "entity_scope": record.entity_scope,
                        "publication_or_effective_at": record.publication_or_effective_at,
                        "values": record.derived_snapshot_features or {},
                        "source_health": record.collection_health}
                       for record in records if record.included]
        for result in qualifications:
            condition = result.canonical_condition()
            if condition is None:
                continue
            comparison_results.extend(self.comparison_engine.compare_condition(
                company_id=identity.company_id, entity_scope=result.entity_scope,
                condition_id=str(condition["condition_id"]),
                condition_type=result.condition_candidate_type, cutoff=as_of, records=raw_records))
        used_peer_ids = {evidence_id for result in comparison_results for evidence_id in result.evidence_ids
                         if evidence_id.startswith("ev-peer-")}
        evidence.extend(row for row in self.comparison_engine.peer_evidence(as_of)
                        if row["evidence_id"] in used_peer_ids)
        comparison = []
        for result in comparison_results:
            included_names = [item.display_name for item in result.cohort_members if item.included]
            excluded_names = [f"{item.display_name}: {item.exclusion_reason}" for item in result.cohort_members if not item.included]
            comparison.append({"comparison_id":result.comparison_id,
                "condition_id":result.qualified_condition_id,"status":result.status,
                "basis":"HISTORY" if result.comparison_type == "OWN_HISTORY" else "PEER",
                "metric":result.feature_definition,"target_value":result.company_value,
                "comparator_value":result.reference_value,"gap":result.gap,"unit":result.feature_unit,
                "percentile":result.percentile,"sample_size":result.sample_size,
                "peer_set_or_history":result.cohort_definition or result.comparison_window,
                "method":result.method,"confidence":"MEDIUM" if result.status == "AVAILABLE" else "NOT_ASSESSED",
                "evidence_ids":result.evidence_ids,"caveats":result.caveats,
                "withheld_reason":None if result.status == "AVAILABLE" else "; ".join(result.missingness),
                "contract_version":result.contract_version,"comparison_type":result.comparison_type,
                "feature_id":result.feature_id,"source_family":result.source_family,
                "comparison_window":result.comparison_window,"directionality":result.directionality,
                "raw_value":result.raw_value,"denominator":result.denominator,
                "anomaly_strength":result.anomaly_strength,"coverage":result.coverage,
                "missingness":result.missingness,"cohort_definition":result.cohort_definition,
                "cohort_inclusions":included_names,"cohort_exclusions":excluded_names})
        ...
        payload = {
            ...
            "capabilities":{"detect":"AVAILABLE" if conditions else "INSUFFICIENT_EVIDENCE",
                "compare":"AVAILABLE" if any(item["status"] == "AVAILABLE" for item in comparison) else "INSUFFICIENT_EVIDENCE",
            ...
            "missing_capabilities":["Scientifically validated operational-condition qualification policy",
                "Broad coverage-normalised peer cohorts beyond registered estate features","Validated predictive-pattern engine",
